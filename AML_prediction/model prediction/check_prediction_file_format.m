function check_prediction_file_format(submission_filename)
    
% submission_template
[table_content] = read_xls_v2('AMLTraining.csv',',',0);
tmp = table_content(2:8:end,:);
tmp(1:179,:)=[];
fprintf('Labels in the template \n')
tabulate(tmp(:,4))

% ground truth
[table_content] = read_xls_v2(submission_filename,',',0);
tmp = table_content(2:8:end,:);
tmp(1:179,:)=[];
fprintf('\nLabels in the submission \n')
tabulate(tmp(:,4))
...
if total_lines-skip_rows <=chunkSize
    ...
    fprintf('lines_left_to_read = %10d',lines_left_to_read);
    while 1
...
%         tmp = parse_txt_lines(ftext{1},sep,column_ind);
        ...
        fprintf('\b\b\b\b\b\b\b\b\b\b%10d',lines_left_to_read);
    end
    fprintf('\n');
    fclose(fid);
end
